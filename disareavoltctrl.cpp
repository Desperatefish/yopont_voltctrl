...
static StrategyMode currentHourStrategy(const DistAreaVoltCtrlParams* p, int hour)
{
    ...
    {
    case 0:
        return (StrategyMode)p->hour0StrategySetting;

    case 1:
        return (StrategyMode)p->hour1StrategySetting;

    case 2:
        return (StrategyMode)p->hour2StrategySetting;

    case 3:
        return (StrategyMode)p->hour3StrategySetting;

    case 4:
        return (StrategyMode)p->hour4StrategySetting;

    case 5:
        return (StrategyMode)p->hour5StrategySetting;

    case 6:
        return (StrategyMode)p->hour6StrategySetting;

    case 7:
        return (StrategyMode)p->hour7StrategySetting;

    case 8:
        return (StrategyMode)p->hour8StrategySetting;

    case 9:
        return (StrategyMode)p->hour9StrategySetting;

    case 10:
        return (StrategyMode)p->hour10StrategySetting;

    case 11:
        return (StrategyMode)p->hour11StrategySetting;

    case 12:
        return (StrategyMode)p->hour12StrategySetting;

    case 13:
        return (StrategyMode)p->hour13StrategySetting;

    case 14:
        return (StrategyMode)p->hour14StrategySetting;

    case 15:
        return (StrategyMode)p->hour15StrategySetting;

    case 16:
        return (StrategyMode)p->hour16StrategySetting;

    case 17:
        return (StrategyMode)p->hour17StrategySetting;

    case 18:
        return (StrategyMode)p->hour18StrategySetting;

    case 19:
        return (StrategyMode)p->hour19StrategySetting;

    case 20:
        return (StrategyMode)p->hour20StrategySetting;

    case 21:
        return (StrategyMode)p->hour21StrategySetting;

    case 22:
        return (StrategyMode)p->hour22StrategySetting;

    case 23:
        return (StrategyMode)p->hour23StrategySetting;

    default:
        return StrategyMode::StrategyStop;
    }
}
...
static float selectTargetVoltage(StrategyMode currentStrategy, float lowTargetVoltage, float highTargetVoltage)
{
    ...
    {
        return highTargetVoltage;   // 优先高压目标
    }
...
static void stepAdjustActivePower(float currentVolt, float targetVoltage, float Deadband, float regulationStep, float currentActivePower, FloatWithValid* outputP,char phase)
{
    ...
    if(qFabs(diff) > Deadband)
    {
        if(diff > 0)
        {
            ...
            outputP->valid = true;
            qInfo() <<phase<< "相高电压,步进减少有功功率" << regulationStep;
        }
        else
        {
            outputP->value = currentActivePower + regulationStep;
            outputP->valid = true;
            qInfo() <<phase<< "相低电压，步进增加有功功率" << regulationStep;
        }
    }
}
...
{
    if (!outputP->valid) // 本周期未准备下发就不处理
    {
        qInfo() << phase << "相功率不下发，不处理";
        return;
    }
    ...
    {
        outputP->value = pMax;
        qInfo() <<"超过额定功率，"<< phase << "相有功功率修正为：" << outputP->value;
    }
    if (outputP->value > 0 && maxDischargeI == 0)
    {
        outputP->value = 0;
        qInfo() <<"BMS禁放，"<< phase << "相有功功率修正为：" << outputP->value;
    }
    ...
    {
        outputP->value = 0;
        qInfo() <<"BMS禁充，"<< phase << "相有功功率修正为：" << outputP->value;
    }
    // 可选：量化
    ...
    qInfo() << phase << "相有功功率最终设定值：" << outputP->value;
}
